...
namespace Visualization
{
    public class WaveFormRenderer
    {
        ...
        public Image Render(WaveStream stream, ISampleProvider provider, IPeakProvider peakProvider, WaveFormRendererSettings settings)
        {  
            int bytesPerSample = (stream.WaveFormat.BitsPerSample / 8);
            var samples = stream.Length / (bytesPerSample);
            var samplesPerPixel = (int)(samples / settings.Width);
            var stepSize = settings.PixelsPerPeak + settings.SpacerPixels;
            peakProvider.Init(provider, samplesPerPixel * stepSize);
            return Render(peakProvider, settings);  
        }
